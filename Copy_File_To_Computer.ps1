...
function CheckSource {
    ...
    if ($(Test-Path $SourcePath -PathType Leaf) -eq  $true) {
        $SourcePath
    }
    else {
        CheckSource
...
if ($(Test-Path $InstallMediaLocation) -eq  $true -and $(Test-Path $("\\$ComputerName\c$\TEMP")) -eq  $true) {
   Write-Output "Are you sure you want to copy $InstallMediaLocation to $("\\$ComputerName\c$\TEMP"))?"
   $Permission = 0
   ...
   if ($Permission -eq 1) {
    ...
    Copy-Item -Path $("$InstallMediaLocation") -Destination $NewFolder
   }
   if ($Permission -eq 1) {
